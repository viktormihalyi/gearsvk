    ...
    def boot(self,
            *,
            ...
            imagePath0      : 'Image path.' 
                            = './Project/Media/noise.png',
            imagePath1      : 'Image path.' 
                            = './Project/Media/gems.png',
            imagePath2      : 'Image path.' 
                            = './Project/Media/rgbNoise.png'
            ):
        self.name                =      name
...
{
	return vec3( 32.0*cos(0.2+0.75*.1*stime*1.5), 1.2, 32.0*sin(0.1+0.75*0.11*stime*1.5) );
}
...
void main(  )
{
	...
	float stime = 23.5+time;
	
    // camera	
	vec3 ro = path( stime );
	vec3 ta = path( stime+1.0 );
	ta.y *= 0.2;
	float roll = 0.3*cos(0.07*stime);
	// camera tx
    ...
    if( gt>0.0 )
    {
	    ...
	    nor.xz  = 0.10*(-1.0 + 2.0*texture2D( iChannel2, 1.5*pos.xz ).xz);
	    nor.xz += 0.15*(-1.0 + 2.0*texture2D( iChannel2, 3.2*pos.xz ).xz);
	    nor.xz += 0.20*(-1.0 + 2.0*texture2D( iChannel2, 6.0*pos.xz ).xz);
		nor = normalize(nor);
