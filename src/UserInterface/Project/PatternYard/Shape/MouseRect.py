...
class MouseRect(Component) : 
    ...
    def applyWithArgs(self,
            patch,
            *,
            size_um: 'Dimensions of the rectangle shape, as a (width, height) pair [um,um].'=(2000, 200),
            facing: "The shape orientation in radians (or 'east', 'northeast', 'north', 'northwest', 'west', 'southwest', 'south', or 'southeast')"='east',
    ...
    #        if( l > 0.001):
    #            self.stimulus.setShaderVector( name='facing', x=deltaX / l,
    #            y=-deltaY / l )
    ...
    def onFrame(self, event):
        ...
        #travelLength = math.fabs(experiment.field_width_um * self.facing[0]) +
        #math.fabs(experiment.field_height_um * self.facing[1]) +
        #self.shapeLength_um
        #
        #currentOffs = self.fmod(time + self.phase, self.radius * 2) -
        #self.radius
        #
        #if self.speed > 0.01 :
        ...
        #    self.radius = travelLength / -self.speed
        #self.phase = self.fmod((currentOffs + self.radius) - time, self.radius
        #* 2)
