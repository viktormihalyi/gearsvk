...
class MultiSpot(Component) : 
    ...
    def applyWithArgs(
        ...
        patch.setShaderColor( name = 'backgroundColor',    red = background[0], green = background[1], blue = background[2] )
        i = 0
        s = '''
                        vec3 shape(vec2 x){ 
        ...
        for spot in spots :
            ...
            patch.setShaderVector( name = 'spotRadius' + str(i),    x = spot.radius , y=spot.innerRadius )
            patch.setShaderVector( name = 'spotPos'+ str(i),    x = spot.position[0], y= spot.position[1] )
            patch.setShaderVector( name = 'spotVel'+ str(i),    x = spot.velocity[0], y= spot.velocity[1] )
            patch.setShaderColor( name = 'spotColor'+ str(i),    red = spot.color[0], green = spot.color[1], blue = spot.color[2] )
            s += '''
        ...
        s += '''
            ...
            '''
        patch.setShaderFunction( name = 'shape', src = s )
